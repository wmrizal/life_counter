...
#define EXAMPLE_PIN_NUM_TOUCH_CS       9
...
static void example_lvgl_touch_cb(lv_indev_drv_t * drv, lv_indev_data_t * data)
{
    if (xSemaphoreTake(touch_mux, 0) == pdTRUE) {
        ...
        bool touchpad_pressed = esp_lcd_touch_get_coordinates(drv->user_data, touchpad_x, touchpad_y, touch_strength, &touchpad_cnt, 1);
...
void app_main(void)
{
    ...
    touch_io_config.scl_speed_hz = 20 * 1000;
    ...
    esp_lcd_panel_io_handle_t tp_io_handle = NULL;
    ESP_ERROR_CHECK(esp_lcd_new_panel_io_i2c_v2(i2c_bus, &touch_io_config, &tp_io_handle));
